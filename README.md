...
本项目是一个 Android Xposed 模块，同时也是 Lyricon 歌词框架的插件模块。采用模块化架构，分为主应用模块（app）和共享模块（share）。
...
3. **签名配置**
   设置环境变量：
   ```bash
   export RELEASE_STORE_FILE="release.jks"
...
3. **歌词目录**
   ...
   可通过 SharedPreferences 手动添加自定义目录。
