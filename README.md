...
以下展示一段包含逐字时间轴、罗马音和翻译的三行 LRC 歌词，经解析后生成的 `RichLyricLine` 数据结构。

**输入歌词（增强 LRC 三行格式）**：

```
[00:15.334] <00:15.334>白<00:16.182>昼<00:17.028>夢<00:17.218> <00:17.218>繋<00:18.131>い<00:18.339>で<00:18.605>い<00:18.870>て<00:19.343>
[00:15.334]ha ku chu u mu   tsu na i de i te
[00:15.334]白日之梦 相互牵系
```
...
RichLyricLine(
    // 行级时间
    begin = 15334L,              // 行开始时间（毫秒）
    end = 19343L,                // 行结束时间（毫秒）
    duration = 4009L,            // 行持续时间（finalize 阶段计算）

    // 主歌词（第 1 行）：含逐字时间轴
    text = "白 昼 夢   繋 い で い て",
    words = listOf(
        LyricWord(begin=15334, end=16182, text="白"),
        LyricWord(begin=16182, end=17028, text="昼"),
        LyricWord(begin=17028, end=17218, text="夢"),
        LyricWord(begin=17218, end=17218, text=" "),   // 词间空格
        LyricWord(begin=17218, end=18131, text="繋"),
        LyricWord(begin=18131, end=18339, text="い"),
        LyricWord(begin=18339, end=18605, text="で"),
        LyricWord(begin=18605, end=18870, text="い"),
...
<tt xmlns="http://www.w3.org/ns/ttml">
  <body>
    <div>
      <p begin="15000ms" end="19343ms">
        <span begin="0ms" end="848ms">白</span>
        <span begin="848ms" end="1694ms">昼</span>
        <span begin="1694ms" end="1884ms">夢</span>
      </p>
      <p begin="15000ms" end="19343ms" role="x-translation">白日之梦</p>
    </div>
  </body>
</tt>
```
